import numpy as np
import tensorflow as tf
import maddpg.common.tf_util as U

from maddpg.common.distributions import make_pdtype
from maddpg import AgentTrainer


def discount_with_dones(rewards, dones, gamma):
    discounted = []
    r = 0
    for reward, done in zip(rewards[::-1], dones[::-1]):
        r = reward + gamma * r
        r = r * (1. - done)
        discounted.append(r)
    return discounted[::-1]


def make_update_exp(vals, target_vals):
    polyak = 1.0 - 1e-2
    expression = []
    for var, var_target in zip(sorted(vals, key=lambda v: v.name), sorted(target_vals, key=lambda v: v.name)):
        expression.append(var_target.assign(polyak * var_target + (1.0 - polyak) * var))
    expression = tf.group(*expression)
    return U.function([], [], updates=[expression])


def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None,
            local_q_func=False, num_units=64, scope="trainer", reuse=tf.AUTO_REUSE):
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]

        # set up placeholders
        obs_ph_n = make_obs_ph_n
        act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action" + str(i)) for i in range(len(act_space_n))]

        p_input = obs_ph_n[p_index]

        p = p_func(p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="p_func", num_units=num_units)
        p_func_vars = U.scope_vars(U.absolute_scope_name("p_func"))

        # wrap parameters in distribution
        act_pd = act_pdtype_n[p_index].pdfromflat(p)

        act_sample = act_pd.sample()
        p_reg = tf.reduce_mean(tf.square(act_pd.flatparam()))

        act_input_n = act_ph_n + []
        act_input_n[p_index] = act_pd.sample()
        q_input = tf.concat(obs_ph_n + act_input_n, 1)
        if local_q_func:
            q_input = tf.concat([obs_ph_n[p_index], act_input_n[p_index]], 1)
        q = q_func(q_input, 1, scope="q_func", reuse=True, num_units=num_units)[:, 0]
        pg_loss = -tf.reduce_mean(q)

        loss = pg_loss + p_reg * 1e-3

        optimize_expr = U.minimize_and_clip(optimizer, loss, p_func_vars, grad_norm_clipping)

        # Create callable functions
        train = U.function(inputs=obs_ph_n + act_ph_n, outputs=loss, updates=[optimize_expr])
        act = U.function(inputs=[obs_ph_n[p_index]], outputs=act_sample)

        act_test = U.function(inputs=[obs_ph_n[p_index]], outputs=p)

        p_values = U.function([obs_ph_n[p_index]], p)

        # target network
        target_p = p_func(p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="target_p_func",
                          num_units=num_units)
        target_p_func_vars = U.scope_vars(U.absolute_scope_name("target_p_func"))
        update_target_p = make_update_exp(p_func_vars, target_p_func_vars)

        target_act_sample = act_pdtype_n[p_index].pdfromflat(target_p).sample()
        target_act = U.function(inputs=[obs_ph_n[p_index]], outputs=target_act_sample)

        return act_test, act, train, update_target_p, {'p_values': p_values, 'target_act': target_act,
                                                       'p_vars': p_func_vars, 'target_p_vars': target_p_func_vars}


def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, weight_ph_n, grad_norm_clipping=None,
            local_q_func=False, scope="trainer", reuse=tf.AUTO_REUSE, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]

        # set up placeholders
        obs_ph_n = make_obs_ph_n
        act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action" + str(i)) for i in range(len(act_space_n))]
        target_ph = tf.placeholder(tf.float32, [None], name="target")

        q_input = tf.concat(obs_ph_n + act_ph_n, 1)
        if local_q_func:
            q_input = tf.concat([obs_ph_n[q_index], act_ph_n[q_index]], 1)
        q = q_func(q_input, 1, scope="q_func", num_units=num_units)[:, 0]
        q_func_vars = U.scope_vars(U.absolute_scope_name("q_func"))
        # q = q * weight_ph_n[0]
        q_loss = tf.reduce_sum(tf.square((q - target_ph) * weight_ph_n[0]))

        # viscosity solution to Bellman differential equation in place of an initial condition
        q_reg = tf.reduce_mean(tf.square(q))
        loss = (q - target_ph) * weight_ph_n[0]#tf.square(q - target_ph) #+ 1e-3 * q_reg

        optimize_expr = U.minimize_and_clip(optimizer, q_loss, q_func_vars, grad_norm_clipping)

        # Create callable functions
        train = U.function(inputs=obs_ph_n + act_ph_n + weight_ph_n + [target_ph], outputs=loss, updates=[optimize_expr])
        q_values = U.function(obs_ph_n + act_ph_n + weight_ph_n, q)

        # target network
        target_q = q_func(q_input, 1, scope="target_q_func", num_units=num_units)[:, 0]
        target_q_func_vars = U.scope_vars(U.absolute_scope_name("target_q_func"))
        update_target_q = make_update_exp(q_func_vars, target_q_func_vars)

        target_q_values = U.function(obs_ph_n + act_ph_n, target_q)

        return train, update_target_q, {'q_values': q_values, 'target_q_values': target_q_values, 'q_vars': q_func_vars,
                                        'target_q_vars': target_q_func_vars}


class Actor(AgentTrainer):
    def __init__(self, name, model, obs_ph_n, act_space_n, agent_index, args, local_q_func=False):
        self.name = name
        self.args = args

        # get tensorflow APIs from p_train
        self.act_test, self.act, self.p_train, self.p_update, self.p_debug = p_train(
            scope=self.name,
            make_obs_ph_n=obs_ph_n,
            act_space_n=act_space_n,
            p_index=agent_index,
            p_func=model,
            q_func=model,
            optimizer=tf.train.AdamOptimizer(learning_rate=args.lr),
            grad_norm_clipping=0.5,
            local_q_func=local_q_func,
            num_units=args.num_units
        )

    def action(self, obs):
        # input: observations from env
        # output: actions from policy
        return self.act(obs[None])[0]

    def action_test(self, obs):
        return self.act_test(obs[None])[0]

    def preupdate(self):
        return

    def update(self, obs_n, act_n):
        p_loss = self.p_train(*(obs_n + act_n))
        self.p_update
        return p_loss


class Critic(AgentTrainer):
    def __init__(self, name, model, obs_ph_n, act_space_n, agent_index, weight_ph_n, args, local_q_func=False):
        self.name = name
        self.args = args

        # get tenesorflow placeholder from q_train
        self.q_train, self.q_update, self.q_debug = q_train(
            scope=self.name,
            make_obs_ph_n=obs_ph_n,
            act_space_n=act_space_n,
            q_index=agent_index,
            q_func=model,
            optimizer=tf.train.AdamOptimizer(learning_rate=args.lr),
            weight_ph_n=weight_ph_n,
            grad_norm_clipping=0.5,
            local_q_func=local_q_func,
            num_units=args.num_units
        )

    def update(self, obs_n, act_n, target_q):  # target_q must not be a list type
        q_loss = self.q_train(*(obs_n + act_n + [target_q]))
        self.q_update()
        return q_loss


class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        self.name = name
        self.n = len(obs_shape_n)

        self.agent_index = agent_index
        self.args = args
        obs_ph_n = []
        for i in range(self.n):
            obs_ph_n.append(U.BatchInput(obs_shape_n[i], name="observation" + str(i)).get())
        weight_ph_n = [U.BatchInput([], name="priority_weights" + str(self.agent_index)).get()]
        # Create all the functions necessary to train the model
        self.critic = Critic(name, model, obs_ph_n, act_space_n, agent_index, weight_ph_n, args, local_q_func)
        self.actor = Actor(name, model, obs_ph_n, act_space_n, agent_index, args, local_q_func)
        # According to the tensorflow scope p_train and q_train, q_train must be in the front of p_train


    def action(self, obs):
        return self.actor.act(obs[None])[0]

    def action_test(self, obs):
        return self.actor.act_test(obs[None])[0]

    def experience(self, obs, act, rew, new_obs, done, gamma):
        # Store transition in the replay buffer.
        self.replay_buffer.add(obs, act, rew, new_obs, done, gamma)


    def update(self, agents, sample, num_sample=1):
        obs_n_a, act_n_a, rew_n_a, obs_next_n_a, done_n_a, gamma_n_a, weight_n_a, idx_n_a = sample
        # collect replay sample from all agents
        obs_n = []
        obs_next_n = []
        act_n = []
        for i in range(self.n):
            obs_n.append(obs_n_a[:, i, :])
            obs_next_n.append(obs_next_n_a[:, i, :])
            act_n.append(act_n_a[:, i, :])
        rew, done = rew_n_a[:, self.agent_index], done_n_a[:, self.agent_index]
        weight, idx = weight_n_a, idx_n_a
        # train q network
        target_q = 0.0
        for j in range(num_sample):
            #####green nodes take ddpg
            # get next act from Actor target network
            target_act_next_n = [agents[i].actor.p_debug['target_act'](obs_next_n[i]) for i in range(self.n)]
            # get next q value from Critic Q target network
            target_q_next = self.critic.q_debug['target_q_values'](*(obs_next_n + target_act_next_n))
            target_q += rew + self.args.gamma * (1.0 - done) * target_q_next
        target_q /= num_sample
        TD_error = self.critic.q_train(*(obs_n + act_n + [weight] + [target_q]))
        # q_values = self.critic.q_debug['q_values'](*(obs_n + act_n + [weight]))
        # train p network
        p_loss = self.actor.p_train(*(obs_n + act_n))

        self.actor.p_update()
        self.critic.q_update()


        return TD_error
        # return [q_loss, p_loss, np.mean(target_q), np.mean(rew), np.mean(target_q_next), np.std(target_q)]

    def get_name(self):
        return self.name

    def get_variable_list(self):
        all_var_list = tf.trainable_variables()
        var_list = []
        for var in all_var_list:
            if self.name == var.name.split("/")[0]:
                var_list.append(var)
        return var_list

    def get_variable_dict(self, sess):
        self.var_dict = {}
        self.var_dict['name'] = self.name
        var_list = self.get_variable_list()
        for var in var_list:
            self.var_dict[var.name] = sess.run(var)
        return self.var_dict

    def load_weights(self, dict, sess, var_list=None):
        if var_list == None:
            var_list = self.get_variable_list()
        for var in var_list:
            var.load(dict[var.name.replace(self.name, dict['name'])], sess)
