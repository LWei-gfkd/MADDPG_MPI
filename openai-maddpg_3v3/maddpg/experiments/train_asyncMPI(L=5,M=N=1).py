...
def train(arglist, PID=None, lock=None):
    ...
    with U.single_threaded_session() as sess:
                    ...
                    if data_recv[0]:
                        ...
                        if (terminal or done) and (len(episode_rewards) % arglist.save_rate == 0):
                            ...
                            if num_adversaries == 0:
                                print("Rank {}, steps: {}, episodes: {}, mean episode reward: {}, time: {}".format(
                                    comm_rank, sample_number, len(episode_rewards), np.mean(episode_rewards[-arglist.save_rate:]),
                                    round(time.time() - t_start, 3)))
                            else:
                                print(
                                    "Rank {}, steps: {}, episodes: {}, mean episode reward: {}, agent episode reward: {}, time: {}".format(
                                        comm_rank, sample_number, len(episode_rewards),
                                        np.mean(episode_rewards[-arglist.save_rate:]),
                                        [np.mean(rew[-arglist.save_rate:]) for rew in agent_rewards],
                                        round(time.time() - t_start, 3)))
                                print("Rank  {}, red win: {}, green win: {}, red all leave: {}, green all leave: {}".format(
                                    comm_rank, red_win, green_win, red_leave, green_leave))
                                ...
                                out = open('1mydata_{}.csv'.format(comm_rank), 'a', newline='')
        ...
        if comm_rank==1:
            ...
            req_list = []
            while True:
                if not wait_flag_1 or not wait_flag_2 or not wait_flag_3 or not wait_flag_4 or not wait_flag_5:
                    if not wait_flag_1:
                    ...
                    if not wait_flag_3:
                        ...
                        wait_flag_3 = True
                    if not wait_flag_4:
                        req4 = comm.irecv(source=5, tag=11)
                        wait_flag_4 = True
                    if not wait_flag_5:
                        req5 = comm.irecv(source=6, tag=11)
                        wait_flag_5 = True
                else:
                    data_recv_1 = req1.test()
                    data_recv_2 = req2.test()
                    data_recv_3 = req3.test()
                    data_recv_4 = req4.test()
                    data_recv_5 = req5.test()
                    if data_recv_1[0] or data_recv_2[0] or data_recv_3[0] or data_recv_4[0] or data_recv_5[0]:
                        if data_recv_1[0]:
                            wait_flag_1 = False
                            if data_recv_1[1] == 'finish':
                                break
                            else:
                                obs_n, action_n, rew_n, new_obs_n, done_n = data_recv_1[1]
                                replay_buffer.add(obs_n, action_n, rew_n, new_obs_n, done_n)
                                sample += 1
                        ...
                        if data_recv_2[0]:
                            ...
                            if data_recv_2[1] == 'finish':
                                ...
                                obs_n, action_n, rew_n, new_obs_n, done_n = data_recv_2[1]
                                replay_buffer.add(obs_n, action_n, rew_n, new_obs_n, done_n)
                                sample += 1
                        ...
                        if data_recv_3[0]:
                            ...
                            if data_recv_3[1] == 'finish':
                                ...
                                obs_n, action_n, rew_n, new_obs_n, done_n = data_recv_3[1]
                                replay_buffer.add(obs_n, action_n, rew_n, new_obs_n, done_n)
                                sample += 1

                        if data_recv_4[0]:
                            wait_flag_4 = False
                            if data_recv_4[1] == 'finish':
                                break
                            else:
                                obs_n, action_n, rew_n, new_obs_n, done_n = data_recv_4[1]
                                replay_buffer.add(obs_n, action_n, rew_n, new_obs_n, done_n)
                                sample += 1

                        if data_recv_5[0]:
                            wait_flag_5 = False
                            if data_recv_5[1] == 'finish':
                                break
                            else:
                                obs_n, action_n, rew_n, new_obs_n, done_n = data_recv_5[1]
                                replay_buffer.add(obs_n, action_n, rew_n, new_obs_n, done_n)
                                sample += 1
