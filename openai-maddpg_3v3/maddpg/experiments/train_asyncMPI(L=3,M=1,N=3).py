    ...
    with U.single_threaded_session() as sess:
        ...
        if 0 <= comm_rank <= (L - 1):
            ...
            while True:
                if not wait_flag:
                    ...
                    req = comm.irecv(350000, source=(comm_rank + 4), tag=11)
                    wait_flag = True
                else:
                    data_recv = req.test()
                    if data_recv[0]:
                        ...
                        if data_recv[1] == 'finish':
                            #finish = True
                            comm.send('finish', dest=L, tag=11)
                            break
                        else:
            ...
            while True:
                if not wait_flag_1 or not wait_flag_2 or not wait_flag_3:
                    if not wait_flag_1:
                        ...
                        wait_flag_1 = True
                    if not wait_flag_2:
                        req2 = comm.irecv(source=1, tag=11)
                        wait_flag_2 = True
                    if not wait_flag_3:
                        req3 = comm.irecv(source=2, tag=11)
                        wait_flag_3 = True
                else:
                    ...
                    if data_recv_1[0] or data_recv_2[0] or data_recv_3[0]:
                        ...
                        if (sample // 100 > 0) and len(replay_buffer) >= arglist.batch_size * arglist.max_episode_len:
                            ...
                            for i in range(N):
                                ...
                                req = comm.isend(send_data, dest=(L + M + i), tag=11)
                                req_list.append(req)
                            for i in range(N):
                                req_list[i].wait()
        ...
        if (L + M) <= comm_rank <= (L + M + N - 1):
            ...
            while True:
                if step >= STEP:
                    #for i in range(L):
                    #    comm.send('finish', dest=i, tag=11)
                    comm.send('finish', dest=(comm_rank - L - M), tag=11)
                    break
                else:
                    start = time.time()
                    data_recv = comm.recv(source=L, tag=11)
                    ...
                    comm.send(param, dest=(comm_rank - L - M), tag=11)
